...
`herdr --remote` runs a local thin client and streams the UI from the remote server. The render encoding controls what crosses the link:
...
- `"semantic"` (default) streams full semantic frames and lets the local client diff them against your terminal. Highest fidelity.
- `"ansi"` streams server-side ANSI diffs, sending only the changed cells. This is lighter over SSH and reduces stutter on remote or higher-latency links.

The `HERDR_RENDER_ENCODING` environment variable overrides this setting (`ansi` / `terminal-ansi` or `semantic`), so you can flip encoding for a single invocation without editing config. The encoding is negotiated once when the client connects, so a change applies on the next `--remote` connection, not mid-session.

If a remote session feels laggy or stuttery, see [smoothing a stuttering remote UI](/docs/persistence-remote/#smoothing-a-stuttering-remote-ui).
